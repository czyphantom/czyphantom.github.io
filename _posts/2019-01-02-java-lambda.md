...
简单来说，Lambda表达式是创建匿名内部类的语法糖，**但也有一点不同，由lamda表达式生成的匿名内部类不显示挂载在任何ClassLoader下，只要当该类没有存在的实例对象、且没有强引用来引用该类的Class对象时，该类就会被GC回收。故而VM Anonymous Class相比于Java语言层面的匿名内部类无需通过ClassClassLoader进行类加载且更易回收。**
