...
从上面的方法我们可以看出，实例化ApplicationContext其实做了很多步骤，省略一些不常用的操作外，实例化应用上下文时，首先取得一个BeanFactory，接着注册Bean的后置处理器（Autowired等注解的实现依靠后置处理器），然后实例化所有单例非延迟加载的Bean。

## 构建BeanFactory

首先来看如何取得BeanFactory，即取得BeanFactory由AbstractRefreshableApplicationContext的refreshBeanFactory方法实现，代码如下：
...
```java
	protected final void refreshBeanFactory() throws BeansException {
		...
		try {
			...
			//beanFactory的id默认是这个上下文的id或者名称
			beanFactory.setSerializationId(getId());
         	//定义是否允许bean定义覆盖，是否允许循环依赖（默认允许），构造方法的循环依赖是不允许的
...
获取BeanFactory的过程中首先判断是否已经有BeanFactory，如果有则先销毁，然后调用createBeanFactory方法创建新的BeanFactory，接着重点是loadBeanDefinition方法加载Bean定义。此方法最终调用AbstractXmlApplicationContext的loadBeanDefinitions方法，最终调用XmlBeanDefinitionReader中的doLoadBeanDefinitions方法,如下：

```java
	//以下是AbstractXmlApplicationContext

	protected void loadBeanDefinitions(DefaultListableBeanFactory beanFactory) throws BeansException, IOException {
		//BeanFactory实际也实现了BeanDefinitionRegistry接口
	...
	//以下是AbstractBeanDefinitionReader
...
加载Bean定义主要根据配置文件抽象为资源，根据资源的inputstream使用DOM方式加载为Document，接着重点是注册Bean定义。注册bean定义最终调用DefaultBeanDefinitionDocumentReader的registerBeanDefinitions方法，代码如下：
...
```java
	...
	//以下是DefaultBeanDefinitionDocumentReader
...
```java
	private void parseDefaultElement(Element ele, BeanDefinitionParserDelegate delegate) {
		...
		//嵌套bean则递归调用doRegisterBeanDefinitions方法
    ...
    public void preInstantiateSingletons() throws BeansException {
      ...
      //得到所有beanName的名称
		...
		for (String beanName : beanNames) {
			//从ConcurrentHashMap中获取指定beanName的BeanDefinition
...
实例化的方式实际上就是先取得所有bean的名称的List，然后根据beanName调用getBean实例化bean，它最后调用AbstractBeanFactory的doGetBean方法,我们只看关键部分的代码：
...
在getBean时，首先会检查是否有该bean的缓存，如果有，直接返回。然后对该bean的depends-on属性的bean进行递归实例化，最后再实例化该bean。实例化bean的方法为createBean,该方法最终调用AbstractAutowireCapableBeanFactory的doCreateBean方法，我们看看关键部分代码：
...
```
注意到resolveBeforeInstantiation方法可以处理有PostProcessor的bean。我们来看看doCreateBean方法的实现：
...
createBean时通过createBeanInstance创建实例，然后处理循环依赖，接着属性装配，然后再处理init-method。先看如何创建实例，也就是createBeanInstance方法：
...
创建bean实例时返回的不是bean的实例而是一个BeanWrapper，根据是否有有参构造器判断使用哪种构造器实例化，实例化时使用策略模式确定使用哪种实例化，如果没有方法覆写则使用反射实例化，否则使用CGLIB实例化。再看属性注入，属性注入由populate方法完成：
   		...
   		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
      		for (BeanPostProcessor bp : getBeanPostProcessors()) {
         		if (bp instanceof InstantiationAwareBeanPostProcessor) {
            		...
            		// 如果返回false，代表不需要进行后续的属性设值，也不需要再经过其他的BeanPostProcessor的处理
